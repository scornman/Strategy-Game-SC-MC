...
public class EpsilonStrategyGameControllerTest {
	...
	public void setUp() {
		...
		observers = setupDefaultObservers();
	}

	/**
	 * Sets up the list of observers to contain a default set of observers for
	 * testing, namely a single StrategyGameReporter object.
	 */
	...
	/**
	 * Make sure a first lt and lt are both defeated when a first lt attacks a
	 * lt when moving one space to attack
	 */
	@Test
	public void firstLtAttacksLtBothRemovedInStandardStrikeMove()
			throws StrategyException {
		swapTwoPiecesInStartConfiguration(PlayerColor.RED, loc80, loc53);// first
																			// lt
																			// -red
		swapTwoPiecesInStartConfiguration(PlayerColor.BLUE, loc79, loc56);// lt
																			// -blue
		...
		controller.move(PieceType.FIRST_LIEUTENANT, loc53, loc54); // move red
																	// first lt
		controller.move(PieceType.LIEUTENANT, loc56, loc55); // move blue lt.
		MoveResult result = controller.move(PieceType.FIRST_LIEUTENANT, loc54,
				loc55); // strike with red first lt.
	...
	/**
	 * Make sure a first lt and lt are both defeated when a first lt attacks a
	 * lt when moving two spaces
	 */
	@Test
	public void firstLtAttacksLtBothRemovedInMoveTwoSpacesStrike()
			throws StrategyException {
		swapTwoPiecesInStartConfiguration(PlayerColor.RED, loc70, loc53);
		...
		MoveResult result = controller.move(PieceType.FIRST_LIEUTENANT, loc56,
				loc54); // strike with blue first lt.
	 ...
	 */
	@Test(expected = StrategyException.class)
	public void firstLtMovesTwoSpacesToEmptySpaceIsNotAllowed()
			throws StrategyException {
	...
	/**
	 * Make sure a first lt cannot attack a piece two spaces away if there is a
	 * player in between the to & from locations
	 */
	@Test(expected = StrategyException.class)
	public void firstLtCannotMoveTwoSpacesToAttackIfAnotherPlayerIsInTheWay()
			throws StrategyException {
		...
		controller.move(PieceType.FIRST_LIEUTENANT, loc47, loc45); // move blue
																	// first lt.
																	// two to
																	// attack
																	// scout
	...
	/**
	 * Makes sure a first lt wins if it attacks a miner two spaces away & moves
	 * to the location of the miner and the miner is removed from the board
	 */
	@Test
	public void blueFirstLtStrikesRedMinerByMovingTwoSpaces()
			throws StrategyException {
		swapTwoPiecesInStartConfiguration(PlayerColor.BLUE, loc89, loc56);
		StrategyGameController controller = factory.makeEpsilonStrategy(
				startingRedConfig, startingBlueConfig, observers);
		...
		assertEquals(new PieceLocationDescriptor(new Piece(
				PieceType.FIRST_LIEUTENANT, PlayerColor.BLUE), loc54),
		...
		// Check that the board has properly updated to reflect the post-battle
		// state.
		assertEquals(new Piece(PieceType.FIRST_LIEUTENANT, PlayerColor.BLUE),
				controller.getPieceAt(loc54));
		assertNull(controller.getPieceAt(loc56));
	 ...
	 * Make sure the marshal's location stays the same & the first lt is removed
	 * from the board
	 */
	@Test
	public void blueFirstLtAttacksRedMarshalAndLoses() throws StrategyException {
		...
		controller.move(PieceType.MARSHAL, loc03, loc04);
		MoveResult result = controller.move(PieceType.FIRST_LIEUTENANT, loc06,
				loc04);
	...
	/**
	 * Makes sure first lt cannot move MORE than two spaces (even with a valid
	 * strike)
	 */
	@Test(expected = StrategyException.class)
	public void firstLtCannotMoveThreeSpacesAndStrike()
			throws StrategyException {
